...
An internal `sf` object with `id`, `name_vi`, `name_en`, `aliases`,
...
<https://www.openstreetmap.org/copyright>. The official baseline used
for the coverage audit is the Foreign Investment Agency (Ministry of
Finance) national industrial-park report of 16 November 2025, recorded
with its source URL in `data-raw/industrial-park-baseline.csv`.
...
`"export_processing_zone"` (khu che xuat) are both counted in the
national industrial-park register, while `"hi_tech_park"` (khu cong nghe
cao) and `"industrial_cluster"` (cum cong nghiep, a provincial tier) are
not.
